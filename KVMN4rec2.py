...
class GRU4Rec:
    ...
    def __init__(self, loss, final_act, hidden_act, layers,
                 ...
                 adapt='adagrad', decay=0.9, grad_cap=0,
                 sigma=0, init_as_normal=False, reset_after_session=True, train_random_order=False, time_sort=True,
                 session_key='SessionId', item_key='ItemId', time_key='Time', out_dim=64, MN_nfactors=10, MN_dims=64):
        self.layers = layers
    ...
    def init(self, data, ItemEmbedding, KBItemEmbedding, r_matrix):
        ...
        ### add memory network
        self.r_matrix = theano.shared(value=np.array(r_matrix, dtype=theano.config.floatX), borrow=True)
        ...
        self.MN_Bea = theano.shared(value=np.zeros((self.MN_dims * 1,), dtype=theano.config.floatX), borrow=True)
        self.constant_ones = theano.shared(value=np.ones((self.MN_dims, ), dtype=theano.config.floatX), borrow=True)
	...
	MN_AW1 = T.nnet.softmax(T.dot(EA, self.r_matrix.T)).dimshuffle(0,1,'x') #shape:b*k*1
        EA = T.repeat(EA.dimshuffle(0,'x',1), self.MN_nfactors, axis=-2) * MN_AW1 #shape:b*k*d
    ...
    def fit(self, data, ItemEmbedding, KBItemEmbedding, r_matrix, retrain=False, sample_store=10000000):
        '''
        Trains the network.
        ...
        if not retrain:
            ...
            offset_sessions = self.init(data, ItemEmbedding, KBItemEmbedding, r_matrix)
        else:
