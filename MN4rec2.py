...
class GRU4Rec:
    ...
    def __init__(self, loss, final_act, hidden_act, layers,
                 n_epochs=10, batch_size=50, dropout_p_hidden=0.5, dropout_p_embed=0.0, learning_rate=0.05, momentum=0.0, lmbd=0.0, embedding=0, KBembedding=0, n_sample=0, sample_alpha=0.75, smoothing=0,
                 adapt='adagrad', decay=0.9, grad_cap=0,
                 sigma=0, init_as_normal=False, reset_after_session=True, train_random_order=False, time_sort=True,
                 session_key='SessionId', item_key='ItemId', time_key='Time', out_dim=64):
        self.layers = layers
    ...
    def init(self, data, ItemEmbedding, KBItemEmbedding):
        ...
        if self.embedding:
            ItemE = self.generate_embedding(ItemEmbedding)
            self.E = theano.shared(ItemE) # shape : self.init_weights((self.n_items, self.embedding))
            n_features = self.embedding
        else:
            n_features = self.n_items
        if self.KBembedding:
            ItemKBE = self.generate_embedding(KBItemEmbedding)
            self.KBE = theano.shared(ItemKBE)
            self.MergeE = theano.shared(value=np.hstack([ItemE, ItemKBE]), borrow=True)            
        else:
            print("error: No KBItemEmbedding ")
        ...
        self.Wy = self.init_weights((self.layers[-1], self.out_dim))
        self.By = theano.shared(value=np.zeros((self.out_dim,), dtype=theano.config.floatX), borrow=True)
        self.WY = self.init_weights((self.KBembedding+self.embedding, self.out_dim))
        self.BY = theano.shared(value=np.zeros((self.out_dim,), dtype=theano.config.floatX), borrow=True)

        ### add memory network
        ## parameters for read operator
        self.MN = theano.shared(value=np.zeros((self.batch_size, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
        self.MN_Wu = self.init_weights((self.layers[-1], self.MN_dims))
        self.MN_Bu = theano.shared(value=np.zeros((self.MN_dims,), dtype=theano.config.floatX), borrow=True)
        self.MN_Wu2 = self.init_weights(((self.MN_dims+self.layers[-1]), self.layers[-1]))
        self.MN_Bu2 = theano.shared(value=np.zeros((self.layers[-1],), dtype=theano.config.floatX), borrow=True)
        ## parameters for write operator
        #self.MN_WI = self.init_weights((self.MN_nfactors, self.MN_dims, self.KBembedding))
        mea = []
        for i in range(self.MN_nfactors):
            mea.append(self.init_matrix((self.KBembedding, self.MN_dims)))        
        self.MN_Wea = theano.shared(value=np.hstack(mea), borrow=True)
        self.MN_Bea = theano.shared(value=np.zeros((self.MN_dims * self.MN_nfactors,), dtype=theano.config.floatX), borrow=True)
        self.constant_ones = theano.shared(value=np.ones((self.MN_dims, 1), dtype=theano.config.floatX), borrow=True)
    ...
    def model(self, X, H, MN, Y=None, drop_p_hidden=0.0, drop_p_embed=0.0, predict=False):
        if self.embedding:
            Sx = self.E[X]
        ...
        EA = (self.hidden_activation(T.dot(KBItem, self.MN_Wea) + self.MN_Bea)).reshape((KBItem.shape[0], self.MN_nfactors, self.MN_dims)) #shape:b*d*1
        MN_gate = T.nnet.sigmoid(T.dot(MN*EA, self.constant_ones))[:,:,0].dimshuffle(0,1,'x') #shape:dot(b*k*d, d*1) = b*k*1
        MN = MN * MN_gate + EA * (1-MN_gate) #shape:b*k*d
        ## read operator
        U_trans = (self.hidden_activation(T.dot(y, self.MN_Wu) + self.MN_Bu)).dimshuffle(0, 1, 'x') # We need to make the same dimension. (shape:b*d*1)
        MN_AW = T.nnet.softmax(T.batched_dot(MN, U_trans)[:,:,0])# shape:b*k
        MN_AC = T.batched_dot(MN.dimshuffle(0,2,1), MN_AW.dimshuffle(0,1,'x')) # shape: b*d*1
        Ut = T.concatenate([y, MN_AC[:,:,0]], axis=-1) # shape:b*(dim+d)
        Ut = self.hidden_activation(T.dot(Ut, self.MN_Wu2) + self.MN_Bu2) #shape : b*layers[-1]
        #H_new[-1] = Ut #self.dropout(Ut, drop_p_hidden)
        # add Dense layer
        y = self.hidden_activation(T.dot(Ut, self.Wy) + self.By)
        if Y is not None:
            Sy = self.MergeE[Y]
            Sy = self.hidden_activation(T.dot(Sy, self.WY) + self.BY)
            #SBy = self.By[Y]
            ...
            return H_new, MN, y, [Sx, Sy]
        else: ## output user embedding
            if predict == True:
                return H_new, MN, y, Sx
            else:
                return "error"
    ...
    def fit(self, data, ItemEmbedding, KBItemEmbedding, retrain=False, sample_store=10000000):
        '''
        Trains the network.
        ...
        if not retrain:
            ...
            offset_sessions = self.init(data, ItemEmbedding, KBItemEmbedding)
        else:
            print("We do not allow retrain, just normal train")
        X = T.ivector()
        Y = T.imatrix()
        H_new, MN_new, Y_pred, sampled_params = self.model(X, self.H, self.MN, Y, self.dropout_p_hidden, self.dropout_p_embed)
        ...
        full_params = [self.Wy, self.By, self.WY, self.BY, self.MN_Wu, self.MN_Bu, self.MN_Wu2, self.MN_Bu2, self.MN_Wea, self.MN_Bea]
        sidxs = [X, Y]
        updates = self.RMSprop(cost, params, full_params, sampled_params, sidxs)
        for i in range(len(self.H)):
            updates[self.H[i]] = H_new[i]
        updates[self.MN] = MN_new
        ...
        for epoch in range(self.n_epochs):
            ...
            self.MN.set_value(np.zeros((self.batch_size, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
            c = []
            ...
            while not finished:
                ...
                #print("new user add"+str(maxiter))
                for idx in mask:
                    maxiter += 1
                ...
                if len(mask) and self.reset_after_session:
                    ...
                    tmp = self.MN.get_value(borrow=True)
                    tmp[mask] = 0
                    self.MN.set_value(tmp, borrow=True)
            avgc = np.mean(c)
            if np.isnan(avgc):
    ...
    def predict_next_batch(self, session_ids, input_item_ids, predict_for_item_ids=None, batch=100):
        ...
        if self.predict is None or self.predict_batch!=batch:
            ...
            self.MN.set_value(np.zeros((batch, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
            #if predict_for_item_ids is not None:
            #    H_new, yhat, _ = self.model(X, self.H, Y, predict=True)
            #else:
            H_new, MN_new, yhat, _ = self.model(X, self.H, self.MN, predict=True) # now, H_new save every layers' embedding, and yhat save the last layer out as user embedding
            
            updatesH = OrderedDict()
            for i in range(len(self.H)):
                updatesH[self.H[i]] = H_new[i]
            updatesH[self.MN] = MN_new
            #if predict_for_item_ids is not None:
    ...
    def save_ItemEmbedding(self, data, filename = 'item_embedding'): #self.itemidmap, self.E
        itemids = data[self.item_key].unique()
        MergeEmbedding = self.hidden_activation(T.dot(self.MergeE, self.WY) + self.BY)#.get_value(borrow=True)
        Embedding = np.asarray(MergeEmbedding.eval())
