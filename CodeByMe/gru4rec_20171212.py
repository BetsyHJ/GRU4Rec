...
Modified on Dec 12 
@author: Betsy
Message: I change the input and output. For the Input, I give the item-embedding pre-trained. And I save the last hidden output (after the GRU and DNN) as user embedding. And finally, we can use dot to calculate the similarity of users' and items'.
"""
...
class GRU4Rec:
	...
	'''
        return updates
    def model1(self, X, H, Y=None, drop_p_hidden=0.0, drop_p_embed=0.0, predict=False):
        if self.embedding:
            Sx = self.E[X]
        ...
        if Y is not None:
            ...
            if predict and self.final_act == 'softmax_logit':
                ...
                y = self.final_activation(T.dot(y, Sy.T))
