...
class GRU4Rec:
    ...
    def init(self, data, ItemEmbedding, KBItemEmbedding):
        ...
        self.MN_WI = self.init_weights((self.KBembedding, self.MN_dims))
        mea = []
        mea.append(self.init_matrix((self.KBembedding, self.MN_dims)))
        mea.append(self.init_matrix((self.KBembedding, self.MN_dims)))        
        self.MN_Wea = theano.shared(value=np.hstack(mea), borrow=True)
        self.MN_Bea = theano.shared(value=np.zeros((self.MN_dims * 2,), dtype=theano.config.floatX), borrow=True)
    ...
    def model(self, X, H, MN, Y=None, drop_p_hidden=0.0, drop_p_embed=0.0, predict=False):
        ...
        EA = T.dot(KBItem, self.MN_Wea) + self.MN_Bea
        KNI_trans = T.dot(KBItem, self.MN_WI).dimshuffle(0, 1, 'x') # shape:b*d*1
        MN_AW2 =  T.repeat(T.batched_dot(MN, KNI_trans), self.MN_nfactors, axis=-1) / self.MN_nfactors #shape:b*k*k
        Erase = T.repeat(EA.T[:self.MN_dims].T.dimshuffle(0,'x',1), self.MN_nfactors, axis=-2) #shape:b*k*d, #Erase = EA[:self.MN_dims] #shape:b*d
        Add = T.repeat(EA.T[self.MN_dims:].T.dimshuffle(0,'x',1), self.MN_nfactors, axis=-2) #Add = EA[self.MN_dims:] #shpe : b*d
        MN = MN - T.batched_dot(MN_AW2, Erase) + T.batched_dot(MN_AW2, Add)
        ## read operator
        U_trans = T.dot(y, self.MN_Wu).dimshuffle(0, 1, 'x') # We need to make the same dimension. (shape:b*d*1)
        MN_AW = T.batched_dot(MN, U_trans) # shape:b*k*1
        MN_AC = T.batched_dot(MN.dimshuffle(0,2,1), MN_AW) # shape: b*d*1
        Ut = T.concatenate([y, MN_AC.dimshuffle(0,1)], axis=-1) # shape:b*(dim+d)
        Ut = self.hidden_activation(T.dot(Ut, self.MN_Wu2) + self.MN_Bu2) #shape : b*layers[-1]
        #H_new[-1] = self.dropout(Ut, drop_p_hidden)
        # add Dense layer
        ...
        if Y is not None:
            ...
            return "error"
    def model_saveUser(self, X, H, MN, Y=None, drop_p_hidden=0.0, drop_p_embed=0.0, predict=False):
        if self.embedding:
            Sx = self.E[X]
            y = self.dropout(Sx, drop_p_embed)
            H_new = []
    ...
    def fit(self, data, ItemEmbedding, KBItemEmbedding, retrain=False, sample_store=10000000):
        ...
        full_params = [self.Wy, self.By, self.WY, self.BY, self.MN_Wu, self.MN_Wu2, self.MN_Bu2, self.MN_Wea, self.MN_Bea, self.MN_WI]
        sidxs = [X, Y]
    ...
    def predict_next_batch(self, session_ids, input_item_ids, predict_for_item_ids=None, batch=100):
        ...
        if self.predict is None or self.predict_batch!=batch:
            ...
            #else:
            H_new, MN_new, yhat, _ = self.model_saveUser(X, self.H, self.MN, predict=True) # now, H_new save every layers' embedding, and yhat save the last layer out as user embedding
